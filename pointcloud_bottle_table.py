...
class PointCloudRansacTest(Node):
    def __init__(self):
        ...
        self.search_z_count = 0
        self.scene_initialized = False
        self.camera_x_offset_px = -25  # 나중에 네가 잰 값 넣기. 예: +35, -20
        ...
        self.side_step = 0.005   # 5mm씩 좌우 이동
        ...
        # [확정 주석 처리] 켜지자마자 제멋대로 초기 자세로 튀는 현상을 방지하기 위해 완전 봉쇄
    ...
    def get_axis_from_contour(self, img, x1, y1, x2, y2):
        roi = img[y1:y2, x1:x2]
        if roi.size == 0:
            return None
        ...
        if np.linalg.norm(v1) >= np.linalg.norm(v2):
            axis_img = v1.astype(np.float64)
        else:
            axis_img = v2.astype(np.float64)

        if np.linalg.norm(axis_img) < 1e-6:
            return None

        axis_img = axis_img / np.linalg.norm(axis_img)
    ...
    def correct_axis_perspective(self, axis_img, center_cam):
        ...
        self.get_logger().warn(
            f"원근 축 보정: yaw={np.degrees(yaw):.1f}, "
            f"pitch={np.degrees(pitch):.1f}, "
            f"corr={np.degrees(correction):.1f}, "
            f"axis_before=({axis_img[0]:.2f},{axis_img[1]:.2f}), "
            f"axis_after=({axis_corr[0]:.2f},{axis_corr[1]:.2f})"
        )
    ...
    def update_planning_scene_cluster_obstacles(self, table_pts_base, bottle_base):
        ...
        floor_box.dimensions = [2.0, 2.0, 0.02]
    ...
    def shutdown_once(self):
        ...
        self.get_logger().warn("작업 완료 → 코드 종료")
        rclpy.shutdown()
    ...
    def send_lift_pose_goal(self, target_pose):
        if not self.move_group_client.wait_for_server(timeout_sec=2.0):
            self.get_logger().error("/move_action server not available")
            return
        ...
        future = self.move_group_client.send_goal_async(goal_msg)
        future.add_done_callback(self.lift_goal_response_callback)

    def lift_goal_response_callback(self, future):
        goal_handle = future.result()

        if not goal_handle.accepted:
            self.get_logger().warn("lift goal rejected")
            return

        self.get_logger().warn("lift goal accepted")

        result_future = goal_handle.get_result_async()
        result_future.add_done_callback(self.lift_result_callback)


    def lift_result_callback(self, future):
        try:
            result = future.result().result

            if result.error_code.val == 1:
                self.get_logger().warn("lift success → return fixed pose")
                self.return_to_fixed_pose_once()
            else:
                self.get_logger().warn(f"lift failed: {result.error_code.val}")

        except Exception as e:
            self.get_logger().warn(f"lift result error: {e}")
    ...
    def color_callback(self, msg):
        ...
        if not found:
            if self.latest_bottle_base is not None or self.goal_active:
                cv2.putText(
                    img,
                    "Bottle not found - but target already locked",
                    (40, 50),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.8,
                    (0, 0, 255),
        ...
        # X 보정이 아닌 일반 3D 계산은 PointCloud 필요
        ...
        if plane_model is not None and bottle_info is not None and table_inliers is not None:
            ...
            if bottle_base is not None and table_inliers is not None:
                self.get_logger().info(f"[STEP 2] 변환 후 (Robot Base Frame): X={bottle_base[0]:.4f}, Y={bottle_base[1]:.4f}, Z(높이)={bottle_base[2]:.4f}\n")
                if abs(bottle_base[0]) > 0.8 or abs(bottle_base[1]) > 0.8 or bottle_base[2] < 0.0 or bottle_base[2] > 1.0:
                ...
                if table_inliers_base is not None:
...
def start_ros_system():
    ...
    "{{trajectory: {{joint_names: [shoulder_pan_joint, shoulder_lift_joint, elbow_joint, wrist_1_joint, wrist_2_joint, wrist_3_joint], points: [{{positions: [4.558644771575928, -0.13444264352832036, -1.6154195070266724, -1.8358494244017542, 1.6432443857192993, -1.6180499235736292], time_from_start: {{sec: 20, nanosec: 0}}}}]}}}}"
    """
